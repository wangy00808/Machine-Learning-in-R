...
cv.ct1 <- train(Outcome ~ . , data = training.data, method = "rpart",
               trControl = train_control, tuneLength = 10, metric = "Kappa")

cv.ct2 <- train(Outcome ~ . , data = training.data, method = "rpart",
               trControl = train_control, tuneGrid = data.frame(cp =seq(0.01,0.05,0.001)))
